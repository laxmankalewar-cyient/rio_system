    ...
    def write_cell(self, row, column, cell_value):
        ...
        if isinstance(cell_value, str):
            if len(cell_value) > self.worksheet.xls_strmax:
                ...
                #cell_value = cell_value.replace("\r", " ") Old Code
                if self.field_names[column] in ['Layout Drawing No.', 'Detail Drawing','DWG Liner Detail','DWG Marking Plan','Detail Drawings','Layout Drawings']:
                    base_url = f'https://rtio-alim/alimweb/Search/QuickLink.aspx?n={cell_value}&t=3&d=Main%5cRio_Tinto_EDM&sc=RTIO&state=LatestApproved&cno=RENDITION&m=l'
                    cell_value = f'=HYPERLINK("{base_url}", "{cell_value}")'
                    cell_style = self.workbook.add_format({'color': 'blue', 'underline': True})
                else:
